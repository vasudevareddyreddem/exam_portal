...

{
	...
	public  function get_exam_list_wise_rank($exam_id){
		...
		foreach($return as $lis){
			...
			$pos = $real_pos = 0;
			$prev_score = -1;
			$cnt=1;foreach ($result as $exam_n => $score) {
				
				$real_pos += 1;// Natural position.
				//echo $score['score'];
				$pos = ($prev_score != $score['score']) ? $real_pos : $pos;
				$results[$score['u_e_id']] = array(
                     "score" => $score['score'], 
                     "position" => $pos, 
                     "exam_no" => $score['exam_id']
                     );
				$datas[$score['u_e_id']]=$score;
				$datas[$score['u_e_id']]['rank']=$pos;
